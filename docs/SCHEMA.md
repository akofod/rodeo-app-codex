...
* `created_by` (uuid, refs profiles.id)
* `created_at` (timestamptz)
* *Note: Recurring events are stored as individual rows in this table.*
...
* `entity_id` (uuid) -- Polymorphic ID (Event ID or Service ID)
* `entity_type` (text) -- "EVENT" or "SERVICE"
* created_at (timestamptz)
